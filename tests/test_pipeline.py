...
def test_unapplied_chase_deposit_routes_to_review(config, ledger):
    qbo = FakeQbo(
    ...
    result = run_scan(qbo, config, ledger, today=TODAY)
    assert result.reminders == []
    assert "possible_unapplied_payment" in kinds(result)
    assert result.unapplied[0].ref == "M1"
    assert result.unapplied[0].confidence == "exact"


def test_unapplied_payment_in_qbo_routes_to_review(config, ledger):
    qbo = FakeQbo(
        invoices=[invoice_row(id="1", txn_date=date(2026, 8, 20))],
...
def test_digest_renders_in_both_formats(config, ledger):
    ...
    assert "1. REMINDERS QUEUED FOR APPROVAL" in text
    assert "2. FLAGGED" in text
    assert "3. POSSIBLE UNAPPLIED PAYMENTS" in text
    assert "4. WHERE AR STANDS" in text
    assert "APPROVE ALL" in text
    assert "<h1" in html and "Reminders queued for approval" in html
